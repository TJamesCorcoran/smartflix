...
class User < ActiveRecord::Base
  ...
  # Temporarily overwrite the display name until it can actually be set
  # through SmartFlix; when it can be set, this should become the
  # default for when empty
  def display_name
  ...
  # NOTE: we will not be authenticating anyone...
  ...
  #  find_by_email_and_hashed_password
  #end
  

  # NOTE: we don't use id, we use customer_id.  Hack'd.
  def self.find_by_id(id)
    find :first, :conditions => { :customer_id => id }
  end
  ...
  # NOTE: Hack to prevent the security hole of allowing an admin bit.  Also easier.
  def admin?
    [225862,200937,200001].include? self.id
  end

  # NOTE: we don't use login names
  def login
    self.email
  end
  ...
  # see also wiki_editor? in railscart/app/models/customer.rb
  def forum_admin?
